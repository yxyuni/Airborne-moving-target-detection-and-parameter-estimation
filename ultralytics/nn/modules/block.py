...
from .InceptionNeXt.block import *
...
class RotationAwareMultiHeadAttention(nn.Module):
    def __init__(self, c1, c2, embed_dim=1024, num_heads=8, num_rotations=2):
        super(RotationAwareMultiHeadAttention, self).__init__()
        self.num_rotations = num_rotations
        self.vertical_conv = nn.Conv2d(c1, c2, kernel_size=(7, 1), padding=(3, 0))
        self.horizontal_conv = nn.Conv2d(c1, c2, kernel_size=(1, 7), padding=(0, 3))
    ...
    def forward(self, x):
        ...
        for i in range(self.num_rotations):
            ...
            rotated_x = torch.rot90(x, i, [2, 3])  # Rotate input
            vertical_features = self.vertical_conv(rotated_x)
            horizontal_features = self.horizontal_conv(rotated_x)
            attention_map = vertical_features + horizontal_features
            rotated_out = attention_map
            rotated_outputs.append(torch.rot90(rotated_out, -i, [2, 3]))  # Rotate back
        return self.sigmoid(torch.stack(rotated_outputs).max(dim=0)[0]) * x + x
            

class FeaturePartitionAttention(nn.Module):
    def __init__(self, in_channels):
        super(FeaturePartitionAttention, self).__init__()
        self.vertical_conv = nn.Conv2d(in_channels, in_channels, kernel_size=(7, 1), padding=(3, 0))
        self.horizontal_conv = nn.Conv2d(in_channels, in_channels, kernel_size=(1, 7), padding=(0, 3))
        self.sigmoid = nn.Sigmoid()
    ...
    def forward(self, x):
        ...
        attention_map = self.sigmoid(vertical_features + horizontal_features)
        return x * attention_map + x

class PKIBlock(nn.Module):
    def __init__(self, in_channels, out_channels, layer_scale=1.0, expansion=1.0, add_identity=True):
        ...
        self.block = MetaNeXtBlock(in_channels)
        self.ffn = ConvMlp(in_channels)
...
class CAA(nn.Module):
    def __init__(self, channels, h_kernel_size=7, v_kernel_size=7):
        super(CAA, self).__init__()
        self.avg_pool = nn.AvgPool2d(7, 1, 3)
        self.conv1 = nn.Sequential(
            nn.Conv2d(channels, channels, 1, 1, 0),
            nn.BatchNorm2d(channels, momentum=0.03, eps=0.001),
            nn.SiLU()
        )
...
class FFA(nn.Module):
    ...
    def __init__(self, c1, c2, channels):
        super().__init__()
        self.conv1 = nn.Conv2d(c1, channels, 3, 1, 1)
        self.conv2 = nn.Conv2d(c1, channels, 3, 1, 2, 2)
        self.conv3 = nn.Conv2d(c1, channels, 3, 1, 3, 3)
        self.conv4 = nn.Conv2d(3*channels, c2)
        self.relu = nn.ReLU()
    ...
    def forward(self, x):
        ...
        return x + self.relu(self.conv4(x4))
...
class DAPPM(nn.Module):
    def __init__(self,
        ...
        self.scales = nn.ModuleList([nn.Sequential(
            ...
            nn.SiLU(),
            nn.Conv2d(self.in_channels, self.branch_channels, kernel_size=1)
        ...
        for i in range(num_scales-1):
            self.process.append(nn.Sequential(
                ...
                nn.SiLU(),
                nn.Conv2d(self.branch_channels, self.branch_channels, kernel_size=3, padding=1)
            ))
        self.compression = nn.Sequential(
            nn.BatchNorm2d(branch_channels * num_scales),
            nn.SiLU(),
            nn.Conv2d(branch_channels * num_scales, self.out_channels, kernel_size=1),
        )
    ...
    def forward(self, inputs):
        ...
        for i in range(1, self.num_scales):
            ...
            # feats.append(self.process[i-1](feats_up + feats[i - 1]))
            feats.append(self.process[i - 1](feats_up))
...
class EcaModule(nn.Module):
    def __init__(self, in_channels):
        ...
        self.kernel_size = self.kernel(in_channels=self.in_channels, b=1, gama=2)
        # self.kernel_size = 5
    ...
    def kernel(self, in_channels, b, gama):
    ...
    def forward(self, x):
        ...
        # x = self.sigmoid(x)
        # return input * x.expand_as(input) + input
...
class RepNCSPELAN5(nn.Module):
    ...
    def forward(self, x):
        ...
        # y[-1] = self.eca(y[-1])
        y.extend((m(y[-1])) for m in [self.cv2, self.cv3])
        return self.eca(self.cv4(torch.cat(y, 1)))
...
class LAE(nn.Module):
    ...
    def __init__(self, ch, c2, st=2, group=16) -> None:
        super().__init__()
        ...
        self.ds_conv = Conv(ch, ch * (st * st), k=3, s=st, g=(ch // group))
        # self.ds_conv = Conv(ch, ch * (st * st), k=3, s=st, g=group)
        # self.ds_conv = Conv(ch, ch * (st * st), k=3, s=st)
        
        self.conv = Conv(ch, c2, k=1, s=1)
    
    def forward(self, x):
        # bs, ch, 2*h, 2*w => bs, ch, h, w, 4
        att = rearrange(self.attention(x), 'bs ch (s1 h) (s2 w) -> bs ch h w (s1 s2)', s1=self.s, s2=self.s)
        att = self.softmax(att)
        
        # bs, 4 * ch, h, w => bs, ch, h, w, 4
        x = rearrange(self.ds_conv(x), 'bs (s ch) h w -> bs ch h w s', s=(self.s * self.s))
